...
def problex_lexicon(corpus: TaggedCorpus) -> torch.Tensor:
    ...
    # add-one smoothing
    tag_counts = torch.ones(V, k)
    word_counts = torch.ones(V)
        ...
        for word, tag in sentence:
            ...
            if tag is not None:
                ...
                if t_idx is not None:
                    tag_counts[w_idx, t_idx] += 1

    tag_probs = tag_counts / tag_counts.sum(dim=1, keepdim=True)
    word_probs = word_counts / word_counts.sum()

    log_tag_probs = torch.log(tag_probs)
    log_word_probs = torch.log(word_probs).unsqueeze(1)

    return torch.cat([log_tag_probs, log_word_probs], dim=1)
