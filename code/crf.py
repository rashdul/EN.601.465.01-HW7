...
class ConditionalRandomField(HiddenMarkovModel):
    ...
    def init_params(self) -> None:
        ...
        if self.unigram:
            self.WA = 0.01 * torch.rand(1, k, device=self.device, dtype=self.dtype)  # unigram model: just one row
        else:
            self.WA = 0.01 * torch.rand(k, k, device=self.device, dtype=self.dtype)  
        self.WB = 0.01 * torch.rand(k, m, device=self.device, dtype=self.dtype)  
    ...
    def updateAB(self) -> None:
        ...
        A_logits = WA_full.to(self.device, self.dtype).clamp(min=-50.0, max=50.0)
        B_logits = self.WB.to(self.device, self.dtype)

        A = torch.exp(A_logits)
        B = torch.exp(B_logits)

        # Structural zeros without in-place ops on tensors needed for autograd.
        A_mask = torch.ones_like(A)
    ...
    def train(self,
        ...
        for evalbatch in more_itertools.batched(
            ...
            if not isnan(sum(learning_speeds)) and len(learning_speeds) > 0:
